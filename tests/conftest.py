...
def _allocate_widths(console: Console, include_function: bool) -> tuple[int, int | None]:
    term_width = console.size.width
    numeric_total = NUM_W * len(STATUS_COLUMNS) + SUBTOTAL_W
    borders = 6 + 3 * (len(STATUS_COLUMNS) + (2 if include_function else 1) + 1)
...
def _make_summary_table(console: Console) -> Table:
    ...
    table = Table(
        title="Pytest Results (summary by file)", expand=False, box=box.SIMPLE_HEAVY, pad_edge=False
    )
    table.add_column(
        "filepath", style=COLOR["filepath"], no_wrap=True, max_width=fp_w, overflow="ellipsis"
    )
    ...
    table.add_column(
        "SUBTOTAL", justify="right", width=SUBTOTAL_W, no_wrap=True, style=COLOR["subtotal"]
    )
    ...
    for fp, cnt in agg.items():
        ...
        row = [f"[{COLOR['filepath']}]{_shorten_middle(fp, fp_w)}[/{COLOR['filepath']}]"]
        row += [f"[{COLOR[c]}]{cnt.get(c, 0)}[/{COLOR[c]}]" for c in STATUS_COLUMNS]
    ...
    total_row += [f"[{COLOR[c]}]{totals.get(c, 0)}[/{COLOR[c]}]" for c in STATUS_COLUMNS]
    total_row.append(f"[{COLOR['subtotal']}]{grand_total}[/{COLOR['subtotal']}]")
...
def _make_full_table(console: Console) -> Table:
    ...
    table = Table(title="Pytest Results (full)", expand=False, box=box.SIMPLE_HEAVY, pad_edge=False)
    table.add_column(
        "filepath", style=COLOR["filepath"], no_wrap=True, max_width=fp_w, overflow="ellipsis"
    )
    table.add_column(
        "function", style=COLOR["func"], no_wrap=True, max_width=fn_w, overflow="ellipsis"
    )
    for col in STATUS_COLUMNS:
        table.add_column(
            labels[col],
...
def pytest_terminal_summary(terminalreporter, exitstatus: int, config: pytest.Config) -> None:
    ...
    if not _records:
        ...
        console.print()
        return
    table = _make_full_table(console) if mode == "full" else _make_summary_table(console)
    console.print(table)
    console.print()
...
def pytest_runtest_makereport(item, call):
    ...
    if test_path is not None:
        ...
        if repo_base:
            url = f"{repo_base}/{rel.as_posix()}" + (f"#L{test_lineno}" if test_lineno else "")
            report.extra.append(_extras.url(url, name="View test source"))
        else:
            report.extra.append(_extras.text(f"Test: {rel}:{test_lineno or ''}"))
    ...
    if test_path and "tests" in test_path.parts:
        try:
            ...
            if name.startswith("test_"):
                ...
                impl_guess = Path(
                    *test_path.parts[:idx], "src", *test_path.parts[idx + 1 : -1], impl
                )
        except ValueError:
            pass
    if impl_guess and impl_guess.exists():
        ...
        if repo_base:
            report.extra.append(
                _extras.url(f"{repo_base}/{rel_impl.as_posix()}", name="View impl (guess)")
            )
        else:
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        items = sorted(hist_dir.glob("tests_*.html"), key=lambda q: q.stat().st_mtime, reverse=True)

        rows = []
        for it in items:
            ts = datetime.fromtimestamp(it.stat().st_mtime).strftime("%Y-%m-%d %H:%M:%S")
            rows.append(f"<tr><td>{ts}</td><td><a href='history/{it.name}'>{it.name}</a></td></tr>")
